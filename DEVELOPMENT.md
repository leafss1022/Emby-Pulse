...
**🎨 前端架构** [→](#frontend)
- [技术选型](#frontend-tech) · [状态管理](#frontend-stores) · [路由配置](#frontend-router) · [页面组件](#frontend-pages) · [布局组件](#frontend-layouts) · [图表组件](#frontend-charts) · [UI组件](#frontend-ui) · [动画系统](#frontend-animation) · [PWA支持](#frontend-pwa)
...
│   │   └── icons/                    # PWA 图标
│   ├── src/
│   │   ├── main.ts                   # Vue 应用入口（含 v-reveal 滚动显现指令）
│   │   ├── App.vue                   # 根组件
│   │   ├── router/
...
│   │   │   └── index.ts              # 硬编码常量、验证规则、动画时长（v2.32.6）
│   │   ├── layouts/
...
│   │       ├── theme.css             # 主题样式
│   │       └── animations.css        # 3D 弧形入场动画定义（v2.32.20）
│   ├── package.json                  # npm 依赖
...
- 统计卡片组件，用于展示数值型统计数据
- 支持图标、标签、数值、颜色配置
- 内置数字动画效果（通过 `AnimatedNumber.vue` 实现）
- 悬停动画和深色模式增强
...
<h3 id="frontend-animation">8. 动画系统 (Animation System)</h3>

#### 滚动显现指令 (v-reveal)
位于 `main.ts`，通过 `IntersectionObserver` 实现元素进入视口时的自动动画。

**核心机制：**
- **全局协调器**：使用 `revealQueue` 队列管理待显示的元素，避免大量元素同时显现造成的视觉冲击。
- **阶梯式显现**：固定 60ms 的交错延迟（电影感黄金分割），确保元素按顺序平滑出现。
- **智能排序**：自动按页面位置（从上到下、从左到右）对同时进入视口的元素进行排序，解决异步加载导致的顺序错乱。
- **性能优化**：`rootMargin` 设置为 85px，提前触发动画；显现后立即 `unobserve`，减少内存占用。

#### 3D 弧形入场动画 (cinematicArcReveal)
定义在 `animations.css`，提供电影级的视觉体验。

- **3D 变换**：结合 `perspective`、`translate3d` 和 `rotateX` 实现弧形入场效果。
- **交互优化**：动画结束后回归标准 2D 坐标系，确保点击判定和悬停效果 100% 准确。
- **模糊过渡**：入场时带有轻微模糊，增强深邃感。

#### 全局路由过渡 (Page Transitions)
- **实现方式**：在 `App.vue` 和 `DefaultLayout.vue` 中使用 `<transition name="page-fade" mode="out-in">`。
- **视觉效果**：采用 `Slide-Fade` 组合，页面切换时伴随 20px 的垂直位移和透明度渐变，消除切换时的生硬感。
- **生命周期协同**：通过路由守卫在切换前清空 `v-reveal` 队列，确保入场动画的纯净性。

#### 状态切换过渡 (State Transitions)
- **骨架屏平滑入场**：`LoadingState.vue` 内置 `fadeIn` 动画，确保页面初始化时视觉平稳。
- **无缝内容切换**：在各页面中使用 `<v-fade-transition mode="out-in">` 包裹加载状态与真实内容。
- **效果**：数据加载完成后，骨架屏平滑淡出，真实内容随后以 3D 弧形优雅入场，彻底消除内容“弹出”的跳动感。

#### 数字动画 (AnimatedNumber.vue)
- 使用 `requestAnimationFrame` 实现平滑的数值滚动。
- 支持 `easeOutCubic` 缓动函数，让数值变化更自然。
- 自动处理千分位格式化。

#### 悬停与反馈
- **hover-lift**：卡片悬停时轻微上浮并放大，增强交互反馈。
- **pulse**：正在播放状态的呼吸灯效果。
- **fadeIn**：页面切换时的整体淡入效果。

<h3 id="frontend-pwa">9. PWA 支持</h3>
...
### v2.32.20 (2025-12-20) - ✨ 电影级 3D 弧形入场动画

- 新增 `v-reveal` 全局滚动显现指令（`main.ts`）
- 新增 `animations.css`：定义 `cinematicArcReveal` 3D 动画
- **新增全局路由过渡**：实现全站页面切换的 `Slide-Fade` 柔和动画效果
- **优化加载体验**：引入骨架屏到真实内容的无缝淡入淡出过渡（Cross-fade）
- **优化骨架屏视觉**：为 `LoadingState` 增加初始淡入效果，消除瞬间弹出的生硬感
- 优化交互体验：动画结束后回归 2D 坐标系，修复点击判定偏移问题
- 优化触发时机：`rootMargin` 设置为 85px，解决页面底部空白问题
- 全站应用：首页总览、内容统计、用户统计、设备统计、历史记录、报告配置均已适配阶梯式入场动画
